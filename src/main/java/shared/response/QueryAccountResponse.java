...
public class QueryAccountResponse extends ResponseBody {
    private final int accountNumber;
    private final String holderName;
    private final Currency currency;
    private final float balance;

    public QueryAccountResponse(boolean status, String errorMessage, int accountNumber, String holderName, Currency currency, float balance) {
    ...
    public static QueryAccountResponse error(String errorMessage) {
    ...
    public static QueryAccountResponse fromBytes(byte[] bytes) {
        int ptr = 0;
        ...
        return new QueryAccountResponse(status, errorMessage, accountNumber, holderName, currency, balance);
    }

    public static void main(String[] args) {
        QueryAccountResponse response = new QueryAccountResponse(true, "errorMessage", 1234, "abc", Currency.USD, -103.34F);
        byte[] bytes = response.toBytes();
        System.out.println(Arrays.toString(bytes));
        System.out.println(QueryAccountResponse.fromBytes(bytes));
    }
